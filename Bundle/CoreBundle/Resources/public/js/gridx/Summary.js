...
=====*/
	...
	return declare(_Module, {
		...
		preload: function(args){
			...
			t._build();
			g.footerNode.appendChild(t.domNode);
			//Add this.domNode to be a part of the grid header
			g.vLayout.register(t, 'domNode', 'footerNode');
            ...
            this.connect(this.grid.body, 'onRender', function(){
                t._build();
            });
		},
        ...
        setSummaryData: function(hash) {
            this.summaryData = hash || {};
        },
        ...
        hasSummaryData : function() {
            if(!this.summaryData)
                return false;
            for(var i in this.summaryData){ if(this.summaryData.hasOwnProperty(i)){return true;}}
            return false;
        },
		...
		_build: function(){
            ...
            if(this.hasSummaryData()) {
                ...
                var sb = ['<table role="presentation" border="0" cellpadding="0" cellspacing="0" class="totalsBarTable"><tr>'];

				//this div if for the checkbox
				sb.push('<td style="width:20px">&nbsp;</td>');

                array.forEach(g._columns, function(col){
                    var storeField = col.field;
                    var summaryValue = (storeField ? t.summaryData[storeField] : '') || '';

                    //use the same style as the cell
                    //TODO introduce a custom setting for that
                    var columnCss = '';
                    var columnDefinition = g.lister.definition.columns[col.id];
                    if(columnDefinition) {
                        columnCss = columnDefinition.columnStyleCss || '';
                    }
                ...
                //the stray td is for the scrollbar
                sb.push('<td style="width:17px">&nbsp;</td></tr></table>');
