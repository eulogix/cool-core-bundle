...
class CoolCrudDataSource extends CoolDataSource implements DataSourceInterface {
    ...
    public function build($parameters=[])
    {
        ...
        foreach($this->tableRelations as $relation) {
            ...
            $limit = $relation->getLimitFields();
            if(is_array($limit) && empty($limit))
                $DSFields = [];
            else {
                if($relation->isView()) {
                     $DSFields = $this->getDSFieldsForView($relation->getView(), $relation->getSchema(), $prefix, $relation->getLambdaFilter());
                     foreach($DSFields as $fieldName => $DSField) {
                         if(in_array($fieldName, $relation->getPKfields()))
                             $DSField->setIsPkInSource(true);
                     }
                } else $DSFields = $this->getDSFieldsFor($relation->getTable(), $relation->getSchema(), $prefix, $relation->getLambdaFilter(), $relation->isRequired());
            }
